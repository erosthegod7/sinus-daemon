...
#
# Routes
#     POST /submit?name=<file>   body = the extraction text. Commits it to inbox/ on GitHub,
#                                runs the model, archives to old/<date>/, returns raw output.
#                                This is how Claude feeds the pipeline: chat -> Railway ->
#                                GitHub -> Railway -> chat. No GitHub write access needed
...
# Optional shared secret for POST /submit. Unset = open endpoint (fine while the URL is
# unpublished; set it the day the URL leaks). Sent as header X-Sinus-Token.
SUBMIT_TOKEN = os.environ.get("SINUS_SUBMIT_TOKEN", "")
MAX_SUBMIT_BYTES = 2_000_000
NAME_OK = re.compile(r"^[A-Za-z0-9._-]{3,120}$")
INBOX = "inbox"
OLD = "old"
OUT = "out"
...
def _run_model(data_path: str, work_dir: str) -> str:
    ...
    try:
        # serve() lives in sinus_daemon, NOT sinus. sinus is the engine (FeaturePipeline,
        # fetch_snapshot_ladder, the fusion classes); sinus_daemon imports from it and owns
        # both search_forever and serve. Importing the wrong one here is exactly the mistake
        # that put a copy of sinus_daemon.py in the repo under the name sinus.py.
...
def submit_and_run(name: str, body: bytes) -> str:
    """Land a file in inbox/ on GitHub, then run it. Two commits on purpose: the first
    proves the raw extraction reached GitHub before anything touched it, the second is
    the run + archive. If the run blows up, the input is still safely committed."""
    if not NAME_OK.match(name):
        raise ValueError(f"bad name {name!r}: letters, digits, . _ - only, 3-120 chars")
    if len(body) > MAX_SUBMIT_BYTES:
        raise ValueError(f"body too large ({len(body)} bytes)")
    gs = _store()
    gs._sync()
    inbox = os.path.join(gs.clone_dir, INBOX)
    os.makedirs(inbox, exist_ok=True)
    with open(os.path.join(inbox, name), "wb") as fh:
        fh.write(body)
    if not gs._commit_push(f"inbox: submitted {name}"):
        raise RuntimeError("could not push the submission to GitHub - not running it")
    _log(f"submitted {name} ({len(body)} bytes) -> {INBOX}/")
    r = process_once(gs)
    return r[1] if r else "submitted, but inbox came back empty on re-read\n"
        ...
        def do_POST(self) -> None:                      # noqa: N802
            try:
                ...
                if path.rstrip("/") != "/submit":
                    return self._send(404, "POST only accepts /submit\n")
                if SUBMIT_TOKEN and self.headers.get("X-Sinus-Token", "") != SUBMIT_TOKEN:
                    return self._send(403, "bad or missing X-Sinus-Token\n")
                params = dict(p.split("=", 1) for p in qs.split("&") if "=" in p)
                name = params.get("name", "")
                if not name:
                    ts = pd.Timestamp.now(tz="America/New_York").strftime("%Y-%m-%d_%H%M")
                    name = f"SINUS_{ts}.md"
                n = int(self.headers.get("Content-Length", "0"))
                body = self.rfile.read(n) if n else b""
                if not body.strip():
                    return self._send(400, "empty body\n")
                return self._send(200, submit_and_run(name, body))
