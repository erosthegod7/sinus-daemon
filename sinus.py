...
def _screen_trial(out, p: Dict[str, Any], cfg: EngineConfig, screen_rounds: int,
    ...
    return score_predictions(eng.predict(val), val.Y)["score"]
...
def search_forever(spot_df: pd.DataFrame, work_dir: str, chain_df=None, tft_epochs: int = 12,
            ...
            if prune:
                ...
                if len(screens) >= prune_warmup and np.isfinite(sc):
                    ...
                    if sc > bar:
                        ...
                        print(f"[daemon] trial {trial:>4}  PRUNED at screen {sc:.4f} > p{prune_percentile:.0f} "
    ...
    print(f"[daemon] stopped after {trial - start_trial} trials this run "
