...
# ----------------------------------------------------------------------------- #
class _C:
    """ANSI colours for the console. Disabled automatically when output is redirected to a
    file or a pipe, so log files stay free of escape codes."""
    RESET = "\033[0m"; BOLD = "\033[1m"
    RED = "\033[38;5;196m"; GREEN = "\033[38;5;46m"; YELLOW = "\033[38;5;226m"
    CYAN = "\033[38;5;51m"; MAGENTA = "\033[38;5;201m"; GREY = "\033[38;5;244m"
    ORANGE = "\033[38;5;208m"; BLUE = "\033[38;5;39m"


def _colour_enabled() -> bool:
    """Colour on unless explicitly disabled.

    Deliberately NOT gated on isatty(). The normal way to run this daemon is
    `python railway_daemon.py 2>&1 | Tee-Object -FilePath run.log`, which makes stdout a pipe —
    an isatty() check would silently strip the colour in exactly the case it is wanted. Set
    SINUS_NO_COLOR=1 when you want a clean log file with no escape codes."""
    if os.environ.get("SINUS_NO_COLOR") == "1" or os.environ.get("NO_COLOR"):
        return False
    return True


def _enable_windows_ansi() -> None:
    """Windows consoles need virtual-terminal processing switched on before ANSI works."""
    if os.name == "nt":
        try:
            import ctypes
...
class ColourFormatter(logging.Formatter):
    """Human-readable coloured console output. Level sets the base colour; a few keywords in
    the message override it, because 'NEW CHAMPION' matters more than the fact it is INFO."""

    # Two colours on purpose. Everything routine is GREEN so it reads as one background stream;
    # RED is reserved for the two events worth looking up for — a champion decision and a prune.
    # More colours than that and nothing stands out, which was the point of the exercise.
    LEVEL = {"DEBUG": _C.GREEN, "INFO": _C.GREEN, "WARNING": _C.GREEN,
             "ERROR": _C.BOLD + _C.RED, "CRITICAL": _C.BOLD + _C.RED}
    KEYWORD = [("champion", _C.BOLD + _C.RED), ("promoted", _C.BOLD + _C.RED),
               ("prune", _C.RED), ("fatal", _C.BOLD + _C.RED), ("traceback", _C.BOLD + _C.RED)]

    def format(self, record: logging.LogRecord) -> str:
        msg = record.getMessage()
        low = msg.lower()
        colour = self.LEVEL.get(record.levelname, "")
        for kw, c in self.KEYWORD:
            if kw in low:
                colour = c
                break
        extras = " ".join(f"{k}={v}" for k, v in record.__dict__.items()
                          if k not in JsonFormatter._SKIP and not k.startswith("_") and k != "event")
        line = f"{self.formatTime(record, '%H:%M:%S')} {msg}" + (f"  {_C.GREY}{extras}{_C.RESET}" if extras else "")
        return f"{colour}{line}{_C.RESET}"
...
def get_logger(name: str = "sinus", json_lines: Optional[bool] = None) -> logging.Logger:
    ...
    if not lg.handlers:
        ...
        # JSON when piped to a file or explicitly asked for; colour when a human is watching.
        use_json = json_lines if json_lines is not None else os.environ.get("SINUS_LOG_JSON", "0") == "1"
        if use_json:
            h.setFormatter(JsonFormatter())
        elif _colour_enabled():
...
def snapshot_physics(ch: pd.DataFrame, spot_map: pd.Series, cfg: PipelineConfig) -> pd.DataFrame:
    ...
    value_cols = ["gex", "vanna", "charm", "call_oi", "put_oi"]
    piv = ch.pivot_table(index="ts_grid", columns="strike", values=value_cols,
                         aggfunc="last", observed=True)
    # pivot_table drops a value column that is entirely NaN. Polygon Starter carries no historical
    # open interest, so call_oi/put_oi (and often vanna/charm) arrive all-NaN on volume-proxy days.
    # Put the missing levels back as NaN so every metric below degrades to NaN instead of raising.
    if not isinstance(piv.columns, pd.MultiIndex):
        return pd.DataFrame(columns=GEX_COLS)
    if [v for v in value_cols if v not in piv.columns.get_level_values(0)]:
        strikes = piv.columns.get_level_values(1).unique().sort_values()
        piv = piv.reindex(columns=pd.MultiIndex.from_product([value_cols, strikes],
...
def premium_flow_features(chain: Optional[pd.DataFrame], flow: Optional[pd.DataFrame], grid: pd.DataFrame,
    ...
    if chain is None:
        ...
        ch = chain.assign(ts_grid=snap_to_grid(chain["ts"], cfg))
        # The chain feed carries its own parity `spot`; the grid's bar spot is the canonical one
        # here, and leaving both in place makes the merge emit spot_x/spot_y instead of `spot`.
        ch = ch.drop(columns=[c for c in ("spot",) if c in ch.columns]).merge(
...
class FeaturePipeline:
    ...
    def save(self, path: Union[str, Path]) -> Path:
        """Persist the fitted scaler + config. Without this a saved model is unusable on new data:
        the trees and TFT were trained on features scaled by TRAIN-split statistics, and any
        other scaling silently feeds them numbers from a different distribution."""
        import pickle
        ...
        with open(p, "wb") as fh:
            pickle.dump({"cfg": self.cfg, "scaler": self.scaler}, fh)
        return p
    ...
    def fit(self, seq: Dict[str, np.ndarray], groups: np.ndarray, past_names: Sequence[str], static_names: Sequence[str],
        ...
        for epoch in range(self.tc.max_epochs):
            ...
            for xp, xs, xk, y in batches(tr, True):
                ...
                opt.step()
                tot += loss.detach().item() * len(y); cnt += len(y)
    ...
    def _evaluate(self, idx: np.ndarray, batches: Callable) -> float:
        ...
        with torch.no_grad():
            for xp, xs, xk, y in batches(idx, False):
                tot += masked_pinball_loss(self.model(xp, xs, xk)["quantiles"], y, self.model.quantiles).detach().item() * len(y)
                cnt += len(y)
...
# ============================================================================= #
# LIVE PREDICTION — champion weights + today's chart + today's ladder
# ============================================================================= #
def fetch_champion_from_github(repo: str, token: str, dest: str, verbose: bool = True) -> Dict[str, Any]:
    """Download the champion directory from the GitHub champion repo without needing git.

    Uses the zipball API so it works anywhere with `requests` — Colab, a phone-driven notebook,
    a fresh machine. Returns champion.json contents; raises if no champion has been promoted.
    """
    if requests is None:
        raise RuntimeError("pip install requests")
    import io
    import zipfile
    r = requests.get(f"https://api.github.com/repos/{repo}/zipball/main",
                     headers={"Authorization": f"Bearer {token}", "Accept": "application/vnd.github+json"},
                     timeout=120, allow_redirects=True)
    if r.status_code == 404:
        raise RuntimeError(f"repo {repo} not found or token lacks access")
    r.raise_for_status()
    z = zipfile.ZipFile(io.BytesIO(r.content))
    names = [n for n in z.namelist() if "/champion/" in n and not n.endswith("/")]
    if not names:
        raise RuntimeError(f"no champion/ directory in {repo} — nothing has been promoted yet")
    os.makedirs(dest, exist_ok=True)
    for n in names:
        rel = n.split("/champion/", 1)[1]
        out = os.path.join(dest, rel)
        os.makedirs(os.path.dirname(out), exist_ok=True)
        with open(out, "wb") as fh:
            fh.write(z.read(n))
    meta = json.load(open(os.path.join(dest, "champion.json")))
    if verbose:
        log.info("champion.fetched", extra={"event": "champion", "trial": meta.get("trial"),
                                            "test_score": meta.get("test_score"), "node": meta.get("node"), "dest": dest})
    return meta


def load_champion_stack(champion_dir: str, spot_history: Optional[pd.DataFrame] = None,
                        verbose: bool = True) -> Tuple[FeaturePipeline, CoreModelingEngine, Dict[str, Any]]:
    """Load (pipeline, engine, meta) from a champion directory.

    The pipeline scaler is read from pipeline.pkl when the daemon saved one. Champions promoted
    before that was added have no pipeline.pkl — for those, pass ``spot_history`` (the same
    2-year 1-min series the daemon trained on) and the scaler is REBUILT by refitting on the
    same data with the same split, which reproduces it exactly. Slower, but correct.
    """
    d = Path(champion_dir)
    meta = json.load(open(d / "champion.json"))
    engine = CoreModelingEngine.load(d)
    pk = d / "pipeline.pkl"
    if pk.exists():
        pipeline = FeaturePipeline.load(pk)
        if verbose:
            log.info("champion.pipeline_loaded", extra={"event": "champion", "source": "pipeline.pkl"})
    else:
        if spot_history is None:
            raise RuntimeError("champion has no pipeline.pkl and no spot_history was given to rebuild the scaler")
        n = int(meta.get("n_sessions") or 0)
        hist = spot_history
        if n:
            days = sorted(hist["ts"].dt.normalize().unique())[-n:]
            hist = hist[hist["ts"].dt.normalize().isin(days)]
        pipeline = FeaturePipeline(PipelineConfig())
        pipeline.fit_transform(hist, None, None)
        if verbose:
            log.warning("champion.pipeline_rebuilt", extra={"event": "champion", "sessions": int(hist["ts"].dt.normalize().nunique()),
                                                            "note": "older champion without pipeline.pkl — scaler refit on history"})
    return pipeline, engine, meta


def fetch_polygon_day(symbol: str, day: Union[str, pd.Timestamp], api_key: str) -> pd.DataFrame:
    """One session of 1-min bars from Polygon → spot_df(ts, spot)."""
...
def predict_live(symbol: str = "SPY", champion_dir: str = "champion", spot_history: Optional[pd.DataFrame] = None,
    ...
    """The daily call: champion weights + today's live chart + today's ladder → multi-horizon targets.

    Steps
        1. load pipeline + engine from the champion directory
        2. pull the last ``context_sessions`` days plus today from Polygon (prior-close and
           rolling features need yesterday; the TFT needs a full look-back window)
        3. transform with the CHAMPION's scaler, build the bundle ending at the latest bar
        4. engine.predict → real lgb / cat / tft experts
        5. fetch today's option ladder, fuse with the physics layer
    Returns the fusion result; prints the call. ``log_csv`` appends one row for scoring later.
    """
    key = _key(api_key)
    pipeline, engine, meta = load_champion_stack(champion_dir, spot_history, verbose)
    lookback = int((meta.get("params") or {}).get("lookback", 60))
    ...
    if today_rows < lookback:
        log.warning("predict_live.short_session", extra={"event": "live", "today_bars": today_rows, "lookback": lookback,
    ...
    if mode == "build":
        ...
        try:
            ...
            chain_df = chain_df[chain_df['ts'] <= spot_df['ts'].max()]
            flow_df = flow_df[flow_df['ts'] <= spot_df['ts'].max()]
        except Exception as _e:
            log.warning('predict_live.chain_unavailable', extra={'err': f'{type(_e).__name__}: {_e}'})
            chain_df, flow_df = None, None
    ...
    fr = out.features_raw.iloc[idx]
    spot = float(meta_row["spot"])
    rv = float(fr["rv_1m"]) if np.isfinite(fr["rv_1m"]) else None
    mtc = float(meta_row["minutes_to_close"])
    lad, spot_px, _ = fetch_snapshot_ladder(symbol, api_key=key, spot=spot, verbose=verbose)
    st = market_state_from_polygon(lad, spot, mtc, rv_1m=rv)
    fusion = EnsembleFusionEngine(FusionConfig(ml_target_kind="retn"))
    res = fusion.predict_final(pred, st, row=len(bundle.row_idx) - 1)
    res["_meta"]["champion"] = {"trial": meta.get("trial"), "test_score": meta.get("test_score"), "node": meta.get("node")}
    # --- magnitude meter (patch_daemon_chain.py) ---
    try:
        from sinus_magnitude import predict_magnitude as _pm, format_meter as _fm
        if os.path.exists(os.path.join(champion_dir, 'mag_up.pkl')):
            _F = out.features if hasattr(out, 'features') else out.features_raw
            _row = _F.iloc[idx].to_numpy() if hasattr(_F, 'iloc') else _F[idx]
            res['_magnitude'] = _pm(champion_dir, _row, meta_row['ts'], spot_df)
            print(_fm(res['_magnitude']))
    except Exception as _e:
        log.warning('predict_live.magnitude_failed', extra={'err': f'{type(_e).__name__}: {_e}'})

    res["_meta"]["bar_ts"] = str(meta_row["ts"])

    if verbose:
        experts_live = [e for e in ("lgb", "cat", "tft") if np.isfinite(res["eod"]["experts"].get(e, np.nan))]
        print(f"[live] champion trial {meta.get('trial')} · test {meta.get('test_score', float('nan')):.4f} · "
              f"ML experts active: {experts_live or 'NONE (physics only)'}")
        print(fusion.format_call(res))
    if log_csv:
        row = {"ts": str(now), "bar": str(meta_row["ts"]), "spot": spot, "champion_trial": meta.get("trial"),
               **{f"{h}_target": res[h]["target"] for h in HORIZONS}, **{f"{h}_dir": res[h]["direction"] for h in HORIZONS},
               **{f"{h}_conf": round(res[h]["confidence"], 3) for h in HORIZONS},
               "pin_strike": res["_physics"]["pin_strike"], "zero_gamma": res["_physics"]["zero_gamma_level"], "regime": res["_physics"]["regime"]}
